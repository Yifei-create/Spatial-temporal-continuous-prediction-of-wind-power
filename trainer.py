...
def load_best_model(args):
    """Load best model from previous year"""
    loss = []
    prev_year_dir = osp.join(args.model_path, args.logname + "-" + str(args.seed), str(args.year - 1))
    for filename in os.listdir(prev_year_dir):
        loss.append(filename[0:-4])
    loss = sorted(loss)
    load_path = osp.join(prev_year_dir, loss[0] + ".pkl")
    ...
    if args.method in ['EAC', 'ScaleShift', 'VariationalScaleShift']:
        if args.year == args.begin_year:
            ...
            for idx in range(args.year - args.begin_year):
...
def load_test_best_model(args):
    ...
    loss = []
    for filename in os.listdir(osp.join(args.model_path, args.logname+"-"+str(args.seed), str(args.year))):
        loss.append(filename[0:-4])
    loss = sorted(loss)
    load_path = osp.join(args.model_path, args.logname+"-"+str(args.seed), str(args.year), loss[0]+".pkl")
...
def train(inputs, args):
    ...
    # Model initialization - ensure model is on GPU
    if args.init and args.year > args.begin_year:
    ...
    for epoch in range(args.epoch):
        ...
        for batch_idx, data in enumerate(train_loader):
            if epoch == 0 and batch_idx == 0:
                ...
                # Verify data is moved to GPU
                args.logger.info(f"[*] Data device before transfer: {data.x.device}")
            
            # Move data to GPU with non_blocking for async transfer
...
def test_model(model, args, testset, pin_memory):
    ...
    with torch.no_grad():
        ...
        for data in testset:
            ...
            # Move to CPU only for final metric calculation
