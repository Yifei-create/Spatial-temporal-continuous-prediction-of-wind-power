...
build_jobs() {
  local dataset graph_variant method warmup logname cmd registered
  JOBS=()

  for dataset in "${DATASETS[@]}"; do
    for graph_variant in "${GRAPH_VARIANTS[@]}"; do
      for method in "${METHOD_CANDIDATES[@]}"; do
...
main() {
  ...
  echo "Graph variants   : ${GRAPH_VARIANTS[*]}"
  ...
  echo "Candidate methods: ${METHOD_CANDIDATES[*]}"
  echo "Total runnable jobs: ${#JOBS[@]}"

  run_phase "STGNN_10METHOD_WARMUP" "${JOBS[@]}"
}
