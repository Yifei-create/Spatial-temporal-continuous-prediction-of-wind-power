...
ALL_METHODS=("STGNNStageResidual" "STGNNInputBias" "STGNN" "EAC" "PatchTST")
WARMUP_METHODS=("STGNNStageResidual" "STGNNInputBias" "EAC")
...
seed = int("${SEED}")
cfg = Config(method="STGNN", seed=seed, dataset=dataset, graph_variant=graph_variant)
...
build_jobs() {
  local dataset graph_variant method warmup logname cmd
  JOBS=()
  ...
  for dataset in "${DATASETS[@]}"; do
    for graph_variant in "${GRAPH_VARIANTS[@]}"; do
      ...
      for method in "${ALL_METHODS[@]}"; do
        if method_uses_warmup "$method"; then
          for warmup in on off; do
            cmd="python main.py --dataset ${dataset} --graph_variant ${graph_variant} --method ${method} --train 1 --data_process 0 --seed ${SEED}"
            logname="${dataset}_${graph_variant}_${method}_warmup_${warmup}"
            if [[ "$warmup" == "off" ]]; then
              cmd="${cmd} --no_warmup 1"
            fi
            cmd="${cmd} --logname ${logname}"
            JOBS+=("${cmd}")
          done
        else
          cmd="python main.py --dataset ${dataset} --graph_variant ${graph_variant} --method ${method} --train 1 --data_process 0 --seed ${SEED} --no_warmup 1"
...
main() {
  ...
  echo "Graphs           : ${GRAPH_VARIANTS[*]}"
  echo "Methods          : ${ALL_METHODS[*]}"
  echo "Warmup methods   : ${WARMUP_METHODS[*]}"
  ...
  run_phase "CORE_MODELS_TRAIN_AND_TEST" "${JOBS[@]}"

  echo "============================================================"
  echo "All core model experiments completed."
  echo "============================================================"
}
